...
import com.ardublock.ui.Compile;

import edu.mit.blocks.codeblocks.Block;
import edu.mit.blocks.renderable.RenderableBlock;
import edu.mit.blocks.workspace.Workspace;

public class CompileCodeButtonListener implements ActionListener
{
	private JFrame parentFrame;
	private Context context;
	private Workspace workspace; 
	
	public CompileCodeButtonListener(JFrame frame, Context context)
	{
	...
	public void actionPerformed(ActionEvent e)
	{
		...
		for (RenderableBlock renderableBlock:renderableBlocks)
		{
			...
			if (!block.hasPlug() && (Block.NULL.equals(block.getBeforeBlockID())))
			{
				if(block.getGenusName().equals("procedure") || block.getGenusName().equals("procedure_interrupt") || block.getGenusName().equals("procedure_interrupt2"))
				{
...
//				}
		...
		for (RenderableBlock renderableBlock : headerBlockSet)
		{
			...
			try
			{
				...
				codeSetup = translator.translateSetup(headerBlock.getBlockID());
			}
			catch (SocketNullException e1)
			{
				// TODO Auto-generated catch block
		...
		if (success)
		{
			...
			System.out.println(codeSetup);
			System.out.println(codeLoop);
			System.out.println(codeProc);
			String code = codeHeader + codeLoopParallel + codeSetup + codeLoop + codeProc;*/
		String code = Compile.Compile(workspace, context , parentFrame);
		if (code != "") {
			context.didCompile(code);
		}
